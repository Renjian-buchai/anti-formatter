...
int main(int argc, char* argv[]) {
      ...
      if (character == '\n' && flags & (1 << 2)) {
        ...
        current_string += ' ';
                 ...
                 (character == '(' || character == ')' || character == '[' ||
                  character == ']' || character == '{' || character == '}' ||
                  character == ';' || character == '|' || character == '&')) {
        if (current_type == token_sep && !(flags & (1 << 4)) &&
            !(flags & (1 << 3))) {
          current_string += character;
        } else {
          /* To be extracted */
          flush_andNew(current_string, current_type, all_tokens, character,
                       token_sep);
          flags &= ~(1 << 2);
        }
      } else {
        (void)(current_type == token_seg ? flags & (1 << 4)
               ? (void)(flags = flags & (1 << 3) ? flags & ~(1 << 3) : flags),
               (void)(flush_andNew(current_string, current_type, all_tokens,
                                   character, token_seg)),
               (void)(flags = character == '#' ? flags | (1 << 2)
                                               : flags & ~(1 << 2))
           : flags & (1 << 3)
               ? ((void)(flags = flags & (1 << 3) ? flags & ~(1 << 3) : flags),
                  (void)(flush_andNew(current_string, current_type, all_tokens,
  ...
  for (size_t i = 0; i < all_tokens.size(); i++) {
    all_tokens[i].contents =
        trim_spaces(all_tokens[i].contents, all_tokens[i].type);
  }
...

  {
    ...
    file_creator(all_tokens, last_type, longest_separatorChain, target_file,
                 white_space, longest_segment);
  }
  target_file.close();
...
std::string trim_spaces(const std::string& input, const uint8_t& token_type,
                        std::string to_return, size_t first_nonSpace) {
  if (input.empty()) goto tor;
  if (token_type == token_seg) {
    first_nonSpace = input.find_first_not_of(' ');
    to_return = first_nonSpace == std::string::npos
                    ? ""
                    : input.substr(first_nonSpace, input.find_last_not_of(' ') -
                                                       first_nonSpace + 1);
    goto tor;
  }
  to_return = "";
...
void file_creator(std::vector<token>& all_tokens, uint8_t& last_type,
                  const uint32_t& longest_separatorChain,
                  std::ofstream& target_file, uint32_t& white_space,
...
void flush_andNew(std::string& current_string, uint8_t& current_type,
                  std::vector<token>& all_tokens, char& character,
                  const uint8_t val) {
  token new_token;
